...
import { ArrowLeft, Users, ChevronDown, ChevronUp, Bookmark, BookmarkCheck } from 'lucide-react';
...
const QuestionCard = ({ q, isAuthenticated }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const res = await savedService.toggleSaveQuestion(q._id, 'role');
...
const RolePrep = () => {
  ...
  useEffect(() => {
    interviewService.getQuestionsByRole(id)
      .then((res) => {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl border border-border-light p-6 mb-6 animate-fade-in">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-text-primary">{role?.name || role?.title || 'Role'}</h1>
              {role?.category && <p className="text-text-secondary text-sm mt-0.5">{role.category}</p>}
        ...
        {filtered.length === 0 ? (
          <EmptyState title="No questions yet" description="Questions for this role will appear here soon." />
        ) : (
